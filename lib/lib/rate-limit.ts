...
type TierEndpoint = 'api' | 'chat' | 'upload' | 'knowledge'
type RateLimitEndpoint = 'api' | 'auth' | 'upload' | 'chat'
...
export const tierRateLimitConfigs: Record<UserTier, Record<TierEndpoint, RateLimitConfig>> = {
  [UserTier.FREE]: {
    ...
    chat: { window: '1m', max: 10 }, // 10 messages per minute
    upload: { window: '1h', max: 5 }, // 5 uploads per hour
    knowledge: { window: '1h', max: 20 }, // 20 knowledge operations per hour
  },
  [UserTier.PREMIUM]: {
    api: { window: '1m', max: 200 }, // 200 requests per minute
    chat: { window: '1m', max: 50 }, // 50 messages per minute
    upload: { window: '1h', max: 50 }, // 50 uploads per hour
    knowledge: { window: '1h', max: 100 }, // 100 knowledge operations per hour
  },
  [UserTier.VIP]: {
    api: { window: '1m', max: 1000 }, // 1000 requests per minute
    chat: { window: '1m', max: 200 }, // 200 messages per minute
    upload: { window: '1h', max: 200 }, // 200 uploads per hour
    knowledge: { window: '1h', max: 500 }, // 500 knowledge operations per hour
  },
}
