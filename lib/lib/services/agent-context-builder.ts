...
export const buildAgentContext = async (options: ContextOptions): Promise<AgentContext> => {
  const { organizationId, agentId, pipelineStageId, userMessage, clientIdentifier, scriptContext, agentInstructions } = options
 ...
 const knowledgeGraphContext = userMessage
  ? await buildKnowledgeGraphContext(organizationId, userMessage)
  : ''
...
const buildKnowledgeGraphContext = async (organizationId: string, userMessage: string): Promise<string> => {
 ...
 const relatedEntities = await getRelatedEntities(organizationId, candidates.slice(0, 5))
