...
async function testStableDeployment() {
  console.log('🎯 ТЕСТ СТАБИЛЬНОГО ПРОДАКШЕН ДЕПЛОЯ');
  console.log('🌐 URL: https://gpt-agent-kwid-p65nofbx5-world-wide-services-62780b79.vercel.app');

  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();

  const baseUrl = 'https://gpt-agent-kwid-p65nofbx5-world-wide-services-62780b79.vercel.app';
  ...
  try {
    ...
    if (protectedUrl.includes('/login')) {
      ...
      console.log('❌ Защита маршрутов не работает');
    }
  ...
  // === РЕЗУЛЬТАТ ===
  console.log('\n' + '='.repeat(60));
  console.log('📊 РЕЗУЛЬТАТ ТЕСТИРОВАНИЯ СТАБИЛЬНОГО ДЕПЛОЯ');
  console.log('=' .repeat(60));
  console.log('🌐 ПРОДАКШЕН URL: https://gpt-agent-kwid-p65nofbx5-world-wide-services-62780b79.vercel.app');
