...
describe('API: /api/agents/[id]/knowledge/[knowledgeId]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  describe('DELETE /api/agents/[id]/knowledge/[knowledgeId]', () => {
    it('should return 401 if not authenticated', async () => {
      ...
      const route = await import('@/app/api/agents/[id]/knowledge/[knowledgeId]/route')
      const request = new NextRequest('http://localhost:3000/api/agents/agent-123/knowledge/knowledge-123', {
        method: 'DELETE',
      })
      const params = Promise.resolve({ id: 'agent-123', knowledgeId: 'knowledge-123' })
    ...
    it('should return 404 if knowledge not found', async () => {
      const { auth } = await import('@/auth')
      const { getAgentById } = await import('@/lib/repositories/agents')
      const { getSupabaseServiceRoleClient } = await import('@/lib/supabase/admin')
