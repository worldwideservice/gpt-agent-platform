...
async function testNewUserLogin() {
  console.log('🚀 Тестирование входа НОВОГО пользователя...');
  
  const browser = await chromium.launch({ headless: false });
  const page = await browser.newPage();
  
  try {
    // Данные нового пользователя (из последнего теста регистрации)
    const testEmail = 'test1762084383084@example.com';
    const testPassword = 'Test123456!';
    
    console.log('🔐 Проверяем вход нового пользователя:');
    ...
    try {
      ...
      if (await sidebar.isVisible()) {
        ...
        if (foundCount > 0) {
          console.log(`🎉 СИСТЕМА ПОЛНОСТЬЮ РАБОТАЕТ!`);
