...
export const GET = async (
  ...
  try {
    ...
    if (!agent) {
      return NextResponse.json(
        { success: false, error: "Агент не найден" },
        { status: 404 },
      );
    }

    const trigger = await getTriggerById(session.user.orgId, triggerId, id);

    if (!trigger) {
      return NextResponse.json(
        { success: false, error: "Триггер не найден" },
        { status: 404 },
      );
    }
...
export const PATCH = async (
  ...
  try {
    ...
    const trigger = await updateTrigger(triggerId, session.user.orgId, id, {
      ...parsed.data,
...
export const DELETE = async (
  ...
  try {
    ...
    await deleteTrigger(session.user.orgId, triggerId, id);
