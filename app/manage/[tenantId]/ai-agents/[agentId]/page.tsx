...
export default async function AgentDetailPage({ params }: AgentDetailPageProps) {
  ...
  const assets = await getAgentAssets(session.user.orgId, params.agentId)
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Workspace: <span className="font-mono">{params.tenantId}</span>
          </p>
        </div>
        <Link href={`/manage/${params.tenantId}/ai-agents`} className="text-sm text-primary hover:underline">
          ← Вернуться к списку
        </Link>
      </div>

      <AgentForm tenantId={params.tenantId} agent={agent} />
      <AgentAssets tenantId={params.tenantId} agentId={agent.id} initialAssets={assets} />
