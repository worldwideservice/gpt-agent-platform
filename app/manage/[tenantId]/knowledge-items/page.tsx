...
/**
 * Страница списка Knowledge Items (Статьи)
...
export default function KnowledgeItemsListPage() {
  ...
  // Получаем список категорий для фильтра (упрощенно - можно расширить)
  ...
  const { result, query } = useList<KnowledgeBaseArticle>({
            ...
            {isLoading ? (
              ...
              articles.map((article: KnowledgeBaseArticle) => (
                <TableRow key={article.id}>
                  <TableCell>
                    <Link
                      href={`/manage/${tenantId}/knowledge-items/${article.id}/edit`}
                  ...
                  <TableCell>
                    {article.categoryId ? (
                      <Badge variant="secondary">
                        {categories.find((c: any) => c.id === article.categoryId)?.name || "Без категории"}
                      </Badge>
                    ) : (
                  ...
                  <TableCell>
                    <Badge variant={article.isPublished ? "default" : "secondary"}>
                      {article.isPublished ? "Да" : "Нет"}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <span className="text-sm text-gray-500">{article.viewsCount || 0}</span>
                  </TableCell>
                  <TableCell>
