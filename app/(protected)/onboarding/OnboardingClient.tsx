...
const steps: OnboardingStep[] = [
  {
    ...
    description:
      "Выберите CRM-систему и синхронизируйте воронки, контакты и сделки",
    icon: Database,
  },
  {
    id: "agent",
    ...
    description:
      "Определите цели, модель и каналы взаимодействия для вашего агента",
    icon: Bot,
  },
  {
    id: "summary",
...
const determineStep = (
  ...
  const crmReady =
    crm.credentialsConfigured &&
    crm.connectionConfigured &&
...
export const OnboardingClient = ({ initialState }: OnboardingClientProps) => {
  ...
  const [crmStatus, setCrmStatus] = useState<OnboardingCrmState>(
    initialState.crm,
  );
  const [agentState, setAgentState] = useState<OnboardingAgentState>(
    initialState.agent,
  );
  ...
  const [currentStep, setCurrentStep] = useState<number>(() =>
    determineStep(initialState.crm, initialState.agent),
  );
  ...
  useEffect(() => {
    if (!crmForm.redirectUri && typeof window !== "undefined") {
      setCrmForm((prev) => ({
        ...prev,
        redirectUri: `${window.location.origin}/integrations/kommo/oauth/callback`,
      }));
    }
  }, [crmForm.redirectUri]);
  ...
  const providerLabel = useMemo(() => {
    const provider = crmProviders.find(
      (item) => item.value === crmForm.provider,
    );
  ...
  const completedStepIds = useMemo(() => {
    ...
    if (
      crmStatus.credentialsConfigured &&
      crmStatus.connectionConfigured &&
      crmStatus.sync?.status === "completed"
    ) {
      ids.push("crm");
    }
    ...
    try {
      for (let attempt = 0; attempt < 40; attempt += 1) {
        ...
        if (syncStatus === "completed") {
          showSuccessToast(
            "CRM подключена",
            "Данные Kommo успешно синхронизированы",
          );
          return state;
        }
        ...
        if (syncStatus === "failed") {
          throw new Error(
            state.crm.sync?.error ?? "Синхронизация завершилась с ошибкой",
          );
        }
          ...
          void pollCrmSync().catch((error) => {
            const message =
              error instanceof Error
                ? error.message
                : "Не удалось завершить синхронизацию CRM";
            setCrmError(message);
            showErrorToast("Ошибка синхронизации CRM", message);
          });
        } else {
          const message =
            result.error ?? "Не удалось подключить Kommo, попробуйте снова";
          setIsAwaitingOAuth(false);
          setCrmError(message);
  ...
  const handleConnectCrm = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      const message =
        "Заполните Client ID, Client Secret, Redirect URI и домен CRM";
      setCrmError(message);
      showErrorToast("Ошибка подключения CRM", message);
      return;
    }
    ...
    try {
      ...
      if (!saveResponse.ok || !saveData.success) {
        throw new Error(
          saveData.error ?? "Не удалось сохранить учетные данные",
        );
      }
      ...
      if (!oauthResponse.ok || !oauthData.success || !oauthData.authUrl) {
        throw new Error(
          oauthData.error ?? "Не удалось получить ссылку авторизации",
        );
      }
      ...
      const message =
        error instanceof Error ? error.message : "Не удалось подключить Kommo";
      setCrmError(message);
      setIsAwaitingOAuth(false);
      showErrorToast("Ошибка подключения CRM", message);
    } finally {
  ...
  const handleAgentSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      showSuccessToast(
        ...
        "AI-агент активирован и может принимать обращения",
      );
    } catch (error) {
      const message =
  ...
  const handleLaunchAgent = async () => {
    ...
    try {
      ...
      if (state.isCompleted) {
        showSuccessToast(
          "Онбординг завершён",
          "Перенаправляем в рабочий дашборд",
        );
        router.push("/");
      } else {
        showErrorToast(
          "Онбординг не завершён",
          "Заполните все шаги перед запуском агента",
        );
      }
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : "Не удалось обновить состояние онбординга";
  ...
  const renderCrmSummary = () => {
    return (
      <div className="space-y-4 text-sm text-gray-700">
        <div>
          <span className="font-medium text-gray-900">Провайдер:</span>{" "}
          {providerLabel}
        </div>
        <div>
          <span className="font-medium text-gray-900">Домен:</span>{" "}
          {crmStatus.connection?.baseDomain ?? "—"}
        </div>
        <div>
        ...
        <div>
          <span className="font-medium text-gray-900">Количество воронок:</span>{" "}
          {crmStatus.sync?.pipelinesCount ?? 0}
        </div>
        {crmStatus.sync?.completedAt && (
          <div>
            <span className="font-medium text-gray-900">Завершено:</span>{" "}
            {crmStatus.sync.completedAt}
          </div>
        )}
  ...
  const renderAgentSummary = () => {
    return (
      <div className="space-y-3 text-sm text-gray-700">
        <div>
          <span className="font-medium text-gray-900">Название агента:</span>{" "}
          {agentState?.name ?? agentForm.name}
        </div>
        <div>
          <span className="font-medium text-gray-900">Модель:</span>{" "}
          {agentState?.defaultModel ?? agentForm.model}
        </div>
        <div>
          <span className="font-medium text-gray-900">Каналы:</span>{" "}
          {(agentForm.channels || []).join(", ")}
        </div>
        <div>
  ...
  const renderStepContent = (stepId: OnboardingStep["id"]) => {
    const crmReady =
      crmStatus.credentialsConfigured && crmStatus.connectionConfigured;
          ...
          <form className="space-y-6" onSubmit={handleConnectCrm}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <KwidInput
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setCrmForm((prev) => ({ ...prev, clientId: e.target.value }))
                }
                required
              />
            ...
            <div className="flex items-center justify-between rounded-lg border border-dashed border-gray-300 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
              <div>
                <p className="font-medium text-gray-900 dark:text-white">
                  Тестовая синхронизация данных
                </p>
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Выгрузим только 20 последних сделок и контактов, чтобы
              ...
              <KwidSwitch
                ...
                onCheckedChange={(value) =>
                  setCrmForm((prev) => ({ ...prev, testSync: value }))
                }
            ...
            <div className="flex items-center justify-end space-x-3">
              ...
              >
                {isSavingCredentials ? (
                  ...
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Ожидаем
                    подтверждение...
                  </>
                ) : crmReady ? (
                  "CRM подключена"
            ...
            {crmStatus.sync && (
              <div className="rounded-lg border border-gray-200 bg-white p-4 text-sm text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300">
                <p className="font-medium text-gray-900 dark:text-white">
                  Статус синхронизации: {crmStatus.sync.status}
                </p>
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  {syncDescription}
                </p>
          ...
          <form className="space-y-6" onSubmit={handleAgentSubmit}>
            ...
            <KwidTextarea
              ...
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                setAgentForm((prev) => ({ ...prev, goal: e.target.value }))
              }
              required
            />
            ...
            <KwidSelect
              ...
              onChange={(value: string) =>
                setAgentForm((prev) => ({ ...prev, schedule: value }))
              }
              options={schedules}
              required
            />
        ...
        return (
          <div className="space-y-6">
            <div className="rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-900/20">
              <h3 className="text-base font-semibold text-green-900 dark:text-green-400">
                CRM успешно подключена
              </h3>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Поддомен: {crmStatus.connection?.baseDomain ?? "—"}
                  </p>
                </div>
    ...
    <div className="mx-auto max-w-5xl space-y-10 py-10">
      <header className="space-y-2">
        <h1 className="text-3xl font-semibold text-gray-900 dark:text-white">
          Добро пожаловать! Настроим систему за 3 шага
        </h1>
        <p className="max-w-3xl text-sm text-gray-600 dark:text-gray-400">
          Сначала подключим CRM и заберём данные о клиентах, затем настроим
          AI-агента и определим, что он будет делать. После этого останется
          запустить агента в работу — всё это займёт не больше 5 минут.
        </p>
      </header>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isActive &&
                        "border-custom-200 bg-custom-50 dark:border-custom-800 dark:bg-custom-900/20",
                      ...
                      <div className="flex items-center space-x-3">
                        <div
                          className={cn(
                            ...
                            isActive
                              ? "border-custom-400 text-custom-600 dark:border-custom-500 dark:text-custom-400"
                              : "border-gray-200 text-gray-500 dark:border-gray-700 dark:text-gray-400",
                            isCompleted &&
                              "border-emerald-300 bg-emerald-100 text-emerald-600 dark:border-emerald-500 dark:bg-emerald-900/20 dark:text-emerald-400",
                          )}
                        >
                          {isCompleted ? (
