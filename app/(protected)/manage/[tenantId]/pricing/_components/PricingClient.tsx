    ...
    <div className="space-y-8">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-500 dark:text-gray-400">
            Выберите подходящий тариф и управляйте лимитами ответов ИИ
          </p>
        </div>
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="space-y-1">
              ...
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">
                ...
                {subscription.tokenQuota.toLocaleString("ru-RU")} ответов ИИ в
                месяц)
              </h2>
              <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-gray-400">
                {subscription.renewsAt && (
                  <span>Активно до: {formatDate(subscription.renewsAt)}</span>
                )}
                <span>
                  Платёжный цикл:{" "}
                ...
                <button
                  type="button"
                  className="text-custom-600 hover:underline dark:text-custom-400"
                >
                  Перейти на годовой
                </button>
              </div>
            ...
            <div className="flex flex-col gap-2 text-sm text-slate-500 dark:text-gray-400">
              <span>
                Использовано: {subscription.tokenUsed.toLocaleString("ru-RU")}{" "}
                из {subscription.tokenQuota.toLocaleString("ru-RU")} (Сбросится:{" "}
                {nextResetDate})
              </span>
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        {plans.map((plan) => {
          const isCurrent =
            subscription && plan.id === subscription.plan.toLowerCase();
          return (
            <div
              key={plan.id}
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold uppercase tracking-wide text-slate-400 dark:text-gray-500">
                    Тариф
                  </p>
              ...
              <div>
                ...
                {billingCycle === "yearly" &&
                  plan.priceYearly < plan.priceMonthly && (
                    <p className="mt-1 text-xs font-semibold text-green-600 dark:text-green-400">
                      ...
                      {Math.round(
                        (1 -
                          (plan.priceYearly * 12) / (plan.priceMonthly * 12)) *
                          100,
                      )}
                      %)
                    </p>
                ...
                {billingCycle === "monthly" && (
                  <p className="mt-1 text-xs text-slate-500 dark:text-gray-400">
                    Итого: ${(plan.priceMonthly * 12).toLocaleString("ru-RU")}
                    /год
                  </p>
                )}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <p className="mt-2 text-sm text-slate-500 dark:text-gray-400">
          Попробуйте любой план без риска. Если вас что-то не устроит в течение
          первых 30 дней, мы вернём деньги. Наша служба поддержки готова помочь
          вам сменить план или отменить подписку в любое время.
        </p>
      </section>
          ...
          <aside className="h-full w-96 bg-white shadow-2xl dark:bg-gray-900">
            ...
            <div className="space-y-3 overflow-y-auto px-6 py-4">
              {mockNotifications.map((notification) => (
                <div
                  key={notification.id}
                  className="rounded-xl border border-slate-200 p-4 dark:border-gray-800 dark:bg-gray-800"
                >
...
const AccordionItem = ({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 dark:border-gray-800 dark:bg-gray-800">
      ...
      >
        {question}
        <span className="text-custom-600 dark:text-custom-400">
          {open ? "−" : "+"}
        </span>
      </button>
      {open ? (
