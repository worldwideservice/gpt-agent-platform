...
export const AgentTrainingPage = ({
  agentId,
  agentName,
  tenantId,
}: AgentTrainingPageProps) => {
    ...
    <div className="space-y-6">
      <header className="space-y-2">
        <h1 className="text-3xl font-semibold text-slate-900 dark:text-white">
          Обучение агента: {agentName}
        </h1>
        <p className="text-sm text-slate-500 dark:text-gray-400">
          Загрузите знания о компании, продукты, услуги, скрипты продаж и
          процессы работы
        </p>
      </header>
      ...
      >
        ...
        <KwidTabsContent value="knowledge" className="space-y-6">
          <KwidSection
            ...
            description="Структурированная информация о компании для обучения агента"
          >
              ...
              <div className="grid grid-cols-2 gap-4">
                <KwidSelect
                  ...
                  onChange={(value: string) =>
                    setKnowledgeForm({
                      ...knowledgeForm,
                      category: value as typeof knowledgeForm.category,
                    })
                  }
          ...
          {/* Список знаний */}
          <KwidSection title={`Загруженные знания (${knowledge.length})`}>
            {isLoading ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center dark:border-gray-800 dark:bg-gray-800">
                ...
                <p className="mt-2 text-sm text-slate-500 dark:text-gray-400">
                  Добавьте знания о компании, чтобы агент мог осмысленно
                  отвечать на вопросы
                </p>
              </div>
            ) : (
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const response = await fetch(
                              `/api/agents/${agentId}/knowledge/${item.id}`,
                              {
                                method: "DELETE",
                              },
                            );
        ...
        <KwidTabsContent value="scripts" className="space-y-6">
          <KwidSection
            ...
            description="Создайте скрипты для разных этапов воронки продаж"
          >
            <div className="space-y-4">
              <div className="border border-slate-200 rounded-xl p-4 space-y-4 dark:border-gray-800">
                <KwidInput
                  ...
                  onChange={(e) =>
                    setScriptForm({ ...scriptForm, title: e.target.value })
                  }
                />
                <div className="grid grid-cols-2 gap-4">
                  <KwidSelect
                    ...
                    onChange={(value: string) =>
                      setScriptForm({
                        ...scriptForm,
                        scriptType: value as SalesScript["scriptType"],
                      })
                    }
                    options={[
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              const response = await fetch(
                                `/api/agents/${agentId}/scripts/${script.id}`,
                                {
                                  method: "DELETE",
                                },
                              );
        ...
        <KwidTabsContent value="files" className="space-y-6">
          <KwidSection
            ...
            description="Загрузите документы о компании, они будут автоматически обработаны и добавлены в базу знаний."
          >
        ...
        <KwidTabsContent value="objections" className="space-y-6">
          <KwidSection
            ...
            description="Создайте готовые ответы на типичные возражения клиентов"
          >
            <div className="space-y-4">
              <div className="border border-slate-200 rounded-xl p-4 space-y-4 dark:border-gray-800">
                <KwidSelect
                  ...
                  onChange={(value: string) =>
                    setObjectionForm({
                      ...objectionForm,
                      objectionType:
                        value as ObjectionResponse["objectionType"],
                    })
                  }
                  options={[
                ...
                <div className="flex items-center justify-end gap-3">
                  ...
                  <KwidButton
                    ...
                    disabled={isSaving || !objectionForm.responseScript.trim()}
                  >
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <button
                          ...
                          aria-label="Удалить"
                          onClick={async () => {
                            if (!confirm("Удалить этот ответ на возражение?"))
                              return;

                            try {
                              const response = await fetch(
                                `/api/agents/${agentId}/objections/${objection.id}`,
                                {
                                  method: "DELETE",
                                },
                              );
        ...
        <KwidTabsContent value="memory" className="space-y-6">
          <KwidSection
            ...
            description="Агент автоматически запоминает важные факты из взаимодействий с клиентами"
          >
            <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center dark:border-gray-800 dark:bg-gray-800">
