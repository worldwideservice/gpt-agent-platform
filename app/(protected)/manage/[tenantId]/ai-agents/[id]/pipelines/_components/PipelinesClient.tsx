...
export const PipelinesClient = ({ agentId, agentName, tenantId: propTenantId }: PipelinesClientProps) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.data) {
            setPipelineSettings(data.data.map((s: any) => ({
              id: s.pipeline_id,
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-4 text-3xl font-semibold text-gray-900 dark:text-white">Настройка воронок</h1>
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">Настройте работу агента с воронками продаж</p>
        </div>
