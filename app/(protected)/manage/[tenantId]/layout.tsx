...
const ManageLayout = async ({ children, params }: ManageLayoutProps) => {
  ...
  if (isDemoMode) {
    ...
    if (tenantData) {
      activeOrganization = organizations.find((org) => org.slug === tenantData.slug) ?? organizations[0] ?? null
    } else {
      // Если tenantId не валидный, используем текущую организацию пользователя
      activeOrganization = organizations.find((organization) => organization.id === session.user.orgId) ?? organizations[0] ?? null
    }

    // Проверяем что пользователь имеет доступ к этой организации
    if (activeOrganization && !organizations.some((org) => org.id === activeOrganization.id)) {
      redirect('/login')
    }
    ...
    <div className="fi-layout min-h-screen bg-slate-50">
      <div className="flex">
        ...
        <div className="fi-main-ctn flex-1 lg:ml-72 xl:ml-80 flex flex-col">
          <Header user={session.user} subscriptionRenewsAt={subscriptionRenewsAt} tenantId={tenantId} />
