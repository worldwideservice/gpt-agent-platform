# Полный список данных и методов Kommo CRM API

**Дата обновления:** 2025-01-26  
**Источник:**

- Официальная документация Kommo API: https://developers.kommo.com/docs/about-kommo-api
- Анализ кода проекта
- Реализованные методы в `lib/lib/crm/kommo.ts`

---

## 📋 Полный список доступных данных

### 1. **Воронки продаж (Pipelines)**

**Endpoint:** `GET /api/v4/leads/pipelines`

**Данные:**

- `id` - ID воронки
- `name` - Название воронки
- `sort` - Порядок сортировки
- `is_main` - Основная воронка (boolean)
- `is_unsorted_on` - Включена ли неразобранная воронка
- `is_archive` - Архивная воронка (boolean)
- `account_id` - ID аккаунта
- `_embedded.statuses` - Этапы воронки:
  - `id` - ID этапа
  - `name` - Название этапа
  - `sort` - Порядок сортировки
  - `color` - Цвет этапа
  - `type` - Тип этапа (0-3)
  - `is_editable` - Редактируемый (boolean)

**Метод в коде:** `getPipelines()`

**Использование:**

- Выбор воронок для работы агента
- Настройка этапов для триггеров
- Фильтрация сделок по воронкам

---

### 2. **Этапы воронок (Pipeline Stages / Statuses)**

**Endpoint:** `GET /api/v4/leads/pipelines/{pipeline_id}`

**Данные:**

- `id` - ID этапа
- `name` - Название этапа
- `sort` - Порядок сортировки
- `color` - Цвет этапа (hex)
- `type` - Тип этапа:
  - `0` - Обычный этап
  - `1` - Успешный этап
  - `2` - Неуспешный этап
  - `3` - Первичный этап
- `is_editable` - Можно ли редактировать
- `pipeline_id` - ID воронки

**Метод в коде:** `getPipelines()` (вложено в `_embedded.statuses`)

**Использование:**

- Настройка триггеров по этапам
- Автоматическое перемещение сделок
- Условия для цепочек

---

### 3. **Сделки (Leads)**

**Endpoints:**

- `GET /api/v4/leads` - Список сделок
- `GET /api/v4/leads/{id}` - Конкретная сделка
- `POST /api/v4/leads` - Создание сделки
- `PATCH /api/v4/leads` - Обновление сделки

**Данные:**

- `id` - ID сделки
- `name` - Название сделки
- `price` - Сумма сделки
- `status_id` - ID этапа
- `pipeline_id` - ID воронки
- `responsible_user_id` - ID ответственного
- `created_at` - Дата создания (timestamp)
- `updated_at` - Дата обновления (timestamp)
- `closed_at` - Дата закрытия (timestamp)
- `closest_task_at` - Ближайшая задача (timestamp)
- `is_deleted` - Удалена (boolean)
- `score` - Оценка сделки
- `account_id` - ID аккаунта
- `loss_reason_id` - ID причины потери
- `custom_fields_values` - Кастомные поля:
  - `field_id` - ID поля
  - `field_name` - Название поля
  - `values` - Массив значений:
    - `value` - Значение
    - `enum_id` - ID значения (для select)
    - `enum_code` - Код значения
- `_embedded.contacts` - Связанные контакты:
  - `id` - ID контакта
  - `is_main` - Основной контакт (boolean)
- `_embedded.companies` - Связанные компании:
  - `id` - ID компании
  - `is_main` - Основная компания (boolean)
- `_embedded.tags` - Теги:
  - `id` - ID тега
  - `name` - Название тега

**Методы в коде:**

- `getLead(leadId)` - Получить сделку
- `getLeads()` - Список сделок
- `createLead(lead)` - Создать сделку
- `updateLead(leadId, lead)` - Обновить сделку
- `searchLeads(query)` - Поиск сделок

**Использование:**

- Получение данных сделки для агента
- Обновление полей сделки
- Перемещение по этапам
- Создание новых сделок

---

### 4. **Контакты (Contacts)**

**Endpoints:**

- `GET /api/v4/contacts` - Список контактов
- `GET /api/v4/contacts/{id}` - Конкретный контакт
- `POST /api/v4/contacts` - Создание контакта
- `PATCH /api/v4/contacts` - Обновление контакта

**Данные:**

- `id` - ID контакта
- `name` - Имя контакта
- `first_name` - Имя
- `last_name` - Фамилия
- `responsible_user_id` - ID ответственного
- `created_by` - ID создателя
- `updated_by` - ID обновившего
- `created_at` - Дата создания (timestamp)
- `updated_at` - Дата обновления (timestamp)
- `is_deleted` - Удален (boolean)
- `account_id` - ID аккаунта
- `custom_fields_values` - Кастомные поля:
  - `field_id` - ID поля
  - `field_name` - Название поля
  - `values` - Массив значений:
    - `value` - Значение
    - `enum_id` - ID значения (для select)
    - `enum_code` - Код значения
- `_embedded.companies` - Связанные компании
- `_embedded.leads` - Связанные сделки
- `_embedded.tags` - Теги

**Методы в коде:**

- `getContact(contactId)` - Получить контакт
- `createContact(contact)` - Создать контакт
- `updateContact(contactId, contact)` - Обновить контакт
- `searchContacts(query)` - Поиск контактов

**Использование:**

- Получение данных контакта для агента
- Обновление полей контакта
- Создание новых контактов

---

### 5. **Кастомные поля (Custom Fields)**

**Endpoint:** `GET /api/v4/custom_fields`

**Данные:**

- `id` - ID поля
- `name` - Название поля
- `type` - Тип поля:
  - `text` - Текст
  - `numeric` - Число
  - `checkbox` - Чекбокс
  - `select` - Выпадающий список
  - `multiselect` - Множественный выбор
  - `date` - Дата
  - `url` - URL
  - `textarea` - Многострочный текст
  - `radiobutton` - Радиокнопка
  - `streetaddress` - Адрес
  - `smart_address` - Умный адрес
  - `birthday` - День рождения
  - `legal_entity` - Юридическое лицо
  - `items` - Товары
  - `org_legal_name` - Название организации
  - `linked_entity` - Связанная сущность
- `element_type` - Тип сущности:
  - `contact` - Контакт
  - `lead` - Сделка
  - `company` - Компания
  - `task` - Задача
- `sort` - Порядок сортировки
- `code` - Код поля
- `is_required` - Обязательное (boolean)
- `is_deletable` - Можно удалить (boolean)
- `is_visible` - Видимое (boolean)
- `is_api_only` - Только для API (boolean)
- `enums` - Значения для select/multiselect (Record<string, string>)
- `nested` - Вложенные поля:
  - `id` - ID вложенного поля
  - `name` - Название
  - `sort` - Порядок

**Методы в коде:**

- `getCustomFields()` - Получить все поля
- `createCustomField(field)` - Создать поле

**Использование:**

- Выбор полей для доступа агента
- Автоматическое обновление полей
- Валидация данных

---

### 6. **Пользователи (Users)**

**Endpoint:** `GET /api/v4/users`

**Данные:**

- `id` - ID пользователя
- `name` - Имя пользователя
- `email` - Email
- `lang` - Язык
- `rights` - Права доступа
- `is_active` - Активен (boolean)
- `is_admin` - Администратор (boolean)
- `is_free` - Бесплатный (boolean)
- `group_id` - ID группы
- `role_id` - ID роли

**Методы в коде:**

- `getUsers()` - Получить список пользователей

**Использование:**

- Назначение ответственных
- Фильтрация по менеджерам
- Аналитика по пользователям

---

### 7. **Задачи (Tasks)**

**Endpoints:**

- `GET /api/v4/tasks` - Список задач
- `POST /api/v4/tasks` - Создание задачи
- `PATCH /api/v4/tasks` - Обновление задачи

**Данные:**

- `id` - ID задачи
- `text` - Текст задачи
- `complete_till` - Срок выполнения (timestamp)
- `task_type_id` - ID типа задачи
- `responsible_user_id` - ID ответственного
- `entity_id` - ID сущности (сделка/контакт/компания)
- `entity_type` - Тип сущности:
  - `leads` - Сделка
  - `contacts` - Контакт
  - `companies` - Компания
- `created_by` - ID создателя
- `updated_by` - ID обновившего
- `created_at` - Дата создания (timestamp)
- `updated_at` - Дата обновления (timestamp)
- `is_completed` - Выполнена (boolean)
- `result` - Результат выполнения

**Методы в коде:**

- `createTask(task)` - Создать задачу
- `updateTask(taskId, task)` - Обновить задачу
- `getTasksByEntity(entityId, entityType)` - Получить задачи по сущности

**Использование:**

- Создание задач для менеджеров
- Автоматизация напоминаний
- Отслеживание выполнения

---

### 8. **Примечания (Notes)** ⭐ КРИТИЧНО ДЛЯ РАБОТЫ С ПИСЬМАМИ

**Endpoints:**

- `GET /api/v4/{entity_type}/{entity_id}/notes` - Список примечаний по сущности
- `GET /api/v4/{entity_type}/notes` - Список примечаний по типу сущности
- `GET /api/v4/notes/{id}` - Получить конкретное примечание
- `POST /api/v4/{entity_type}/{entity_id}/notes` - Создание примечания
- `PATCH /api/v4/notes` - Обновление примечаний (массовое)
- `PATCH /api/v4/notes/{id}` - Обновление конкретного примечания

**Данные:**

- `id` - ID примечания
- `entity_id` - ID сущности
- `entity_type` - Тип сущности:
  - `leads` - Сделка
  - `contacts` - Контакт
  - `companies` - Компания
- `note_type` - Тип примечания:
  - `common` - Обычное текстовое примечание
  - `call_in` - Входящий звонок
  - `call_out` - Исходящий звонок
  - `meeting` - Встреча
  - `mail_message` - **Email сообщение (входящее/исходящее)**
  - `outgoing_email` - **Исходящее письмо (альтернативный тип)**
  - `incoming_email` - **Входящее письмо (альтернативный тип)**
  - `sms_in` - Входящее SMS
  - `sms_out` - Исходящее SMS
  - `whatsapp_message` - WhatsApp сообщение
- `params` - Параметры примечания:
  - `text` - Текст (для всех типов)
  - `html` - HTML содержимое (для email)
  - `subject` - Тема письма (для email)
  - `from` - От кого (для email/SMS)
  - `to` - Кому (для email/SMS)
  - `cc` - Копия (для email)
  - `bcc` - Скрытая копия (для email)
  - `status` - Статус (для email: 1=отправлено, 0=черновик)
  - `duration` - Длительность (для звонков, в секундах)
  - `source` - Источник
  - `link` - Ссылка
  - `phone` - Телефон (для звонков/SMS)
  - `uniq` - Уникальный идентификатор
  - `attachments` - Вложения (для email)
- `created_by` - ID создателя
- `updated_by` - ID обновившего
- `created_at` - Дата создания (timestamp)
- `updated_at` - Дата обновления (timestamp)

**Методы в коде:**

- `createNote(note)` - Создать примечание
- `getNotesByEntity(entityId, entityType)` - Получить примечания по сущности
- `sendEmailFromLead(leadId, emailData)` - **Отправить email через сделку**
- `createCallNote(entityId, entityType, callData)` - Создать примечание о звонке
- `createMeetingNote(entityId, entityType, meetingData)` - Создать примечание о встрече

**📧 Работа с электронными письмами:**

**Отправка письма:**

```typescript
// Отправка письма клиенту через сделку
await kommoApi.sendEmailFromLead(leadId, {
  to: ["client@example.com"],
  subject: "Ответ на ваш вопрос",
  html: "<p>Текст письма в HTML</p>",
  text: "Текст письма в plain text",
  from: "agent@company.com",
  cc: ["manager@company.com"],
  bcc: [],
});
```

**Чтение писем из сделки:**

```typescript
// Получить все примечания (включая письма) по сделке
const notes = await kommoApi.getNotesByEntity(leadId, "leads");

// Фильтровать только письма
const emails = notes.filter(
  (note) =>
    note.note_type === "mail_message" ||
    note.note_type === "outgoing_email" ||
    note.note_type === "incoming_email",
);

// Получить входящие письма
const incomingEmails = emails.filter(
  (email) => email.params.from && !email.params.to?.includes(agentEmail),
);

// Получить исходящие письма
const outgoingEmails = emails.filter(
  (email) => email.params.to && email.params.status === 1,
);
```

**Использование:**

- ✅ **Отправка писем клиентам через AI-агента**
- ✅ **Чтение входящих писем из сделки**
- ✅ **История переписки в сделке**
- ✅ **Логирование всех коммуникаций**
- ✅ **Автоматические ответы на письма**

---

### 9. **Компании (Companies)**

**Endpoints:**

- `GET /api/v4/companies` - Список компаний
- `GET /api/v4/companies/{id}` - Конкретная компания
- `POST /api/v4/companies` - Создание компании
- `PATCH /api/v4/companies` - Обновление компании

**Данные:**

- `id` - ID компании
- `name` - Название компании
- `responsible_user_id` - ID ответственного
- `created_at` - Дата создания (timestamp)
- `updated_at` - Дата обновления (timestamp)
- `custom_fields_values` - Кастомные поля
- `_embedded.contacts` - Связанные контакты
- `_embedded.leads` - Связанные сделки
- `_embedded.tags` - Теги

**Методы в коде:**
- `getCompanies()` - Получить список компаний
- `getCompany(companyId)` - Получить компанию по ID
- `createCompany(company)` - Создать компанию
- `updateCompany(companyId, company)` - Обновить компанию

**Использование:**

- Работа с компаниями
- Связывание контактов и сделок

---

### 10. **Теги (Tags)**

**Endpoint:** `GET /api/v4/tags`

**Данные:**

- `id` - ID тега
- `name` - Название тега
- `color` - Цвет тега

**Методы в коде:**
- `getCompanies()` - Получить список компаний
- `getCompany(companyId)` - Получить компанию по ID
- `createCompany(company)` - Создать компанию
- `updateCompany(companyId, company)` - Обновить компанию

**Использование:**

- Группировка сделок/контактов
- Фильтрация

---

### 11. **Аккаунт (Account)**

**Endpoint:** `GET /api/v4/account`

**Данные:**

- `id` - ID аккаунта
- `name` - Название аккаунта
- `subdomain` - Поддомен
- `created_at` - Дата создания (timestamp)
- `currency` - Валюта
- `timezone` - Часовой пояс
- `timezone_offset` - Смещение часового пояса

**Методы в коде:**
- `getCompanies()` - Получить список компаний
- `getCompany(companyId)` - Получить компанию по ID
- `createCompany(company)` - Создать компанию
- `updateCompany(companyId, company)` - Обновить компанию

**Использование:**

- Проверка подключения
- Получение настроек аккаунта

---

### 12. **Статистика (Stats)**

**Методы в коде:**

- `getLeadsStats()` - Статистика по сделкам:
  - `total` - Всего сделок
  - `by_status` - По этапам (Record<status_id, count>)
  - `by_pipeline` - По воронкам (Record<pipeline_id, count>)

**Использование:**

- Аналитика
- Отчеты
- Dashboard

---

### 13. **Входящие лиды (Incoming Leads)**

**Endpoints:**

- `GET /api/v4/incoming_leads` - Список входящих лидов
- `GET /api/v4/incoming_leads/{uid}` - Получить входящий лид по UID
- `POST /api/v4/incoming_leads` - Добавить входящий лид
- `POST /api/v4/incoming_leads/sip` - Добавить входящий лид из звонка
- `POST /api/v4/incoming_leads/form` - Добавить входящий лид из формы
- `POST /api/v4/incoming_leads/{uid}/accept` - Принять входящий лид
- `POST /api/v4/incoming_leads/{uid}/decline` - Отклонить входящий лид
- `POST /api/v4/incoming_leads/{uid}/link` - Привязать входящий лид к сделке/контакту

**Данные:**

- `uid` - Уникальный идентификатор входящего лида
- `source_uid` - UID источника
- `source_type` - Тип источника (form, sip, etc.)
- `pipeline_id` - ID воронки
- `status_id` - ID этапа
- `created_at` - Дата создания
- `metadata` - Метаданные лида

**Методы в коде:**
- `getCompanies()` - Получить список компаний
- `getCompany(companyId)` - Получить компанию по ID
- `createCompany(company)` - Создать компанию
- `updateCompany(companyId, company)` - Обновить компанию

**Использование:**

- Обработка входящих заявок
- Автоматическое создание сделок
- Интеграция с формами и звонками

---

### 14. **Источники (Sources)**

**Endpoints:**

- `GET /api/v4/leads/sources` - Список источников
- `GET /api/v4/leads/sources/{id}` - Получить источник по ID
- `POST /api/v4/leads/sources` - Добавить источник
- `PATCH /api/v4/leads/sources` - Обновить источники (массово)
- `PATCH /api/v4/leads/sources/{id}` - Обновить источник
- `DELETE /api/v4/leads/sources` - Удалить источники (массово)
- `DELETE /api/v4/leads/sources/{id}` - Удалить источник

**Данные:**

- `id` - ID источника
- `name` - Название источника
- `type` - Тип источника
- `external_id` - Внешний ID

**Методы в коде:**
- `getCompanies()` - Получить список компаний
- `getCompany(companyId)` - Получить компанию по ID
- `createCompany(company)` - Создать компанию
- `updateCompany(companyId, company)` - Обновить компанию

**Использование:**

- Отслеживание источников сделок
- Аналитика по источникам
- Настройка каналов

---

### 15. **Шаблоны (Templates)**

**Endpoints:**

- `GET /api/v4/templates` - Список шаблонов
- `GET /api/v4/templates/{id}` - Получить шаблон по ID
- `POST /api/v4/templates` - Добавить шаблон
- `PATCH /api/v4/templates` - Обновить шаблоны (массово)
- `PATCH /api/v4/templates/{id}` - Обновить шаблон
- `POST /api/v4/templates/{id}/submit` - Отправить WhatsApp шаблон на модерацию
- `PATCH /api/v4/templates/{id}/status` - Изменить статус WhatsApp шаблона
- `DELETE /api/v4/templates` - Удалить шаблоны (массово)
- `DELETE /api/v4/templates/{id}` - Удалить шаблон

**Данные:**

- `id` - ID шаблона
- `name` - Название шаблона
- `type` - Тип шаблона (email, sms, whatsapp)
- `subject` - Тема (для email)
- `text` - Текст шаблона
- `html` - HTML содержимое
- `status` - Статус (для WhatsApp)

**Методы в коде:**

- `getEmailTemplates()` - Получить шаблоны писем (заглушка, возвращает пустой массив)

**Использование:**

- Шаблоны писем для агента
- Автоматические ответы
- Персонализация сообщений

---

### 16. **Разговоры (Conversations)**

**Endpoints:**

- `GET /api/v4/conversations/{id}` - Получить разговор по ID
- `POST /api/v4/conversations/{id}/close` - Закрыть разговор

**Данные:**

- `id` - ID разговора
- `entity_id` - ID сущности
- `entity_type` - Тип сущности
- `channel` - Канал связи
- `status` - Статус разговора
- `messages` - Сообщения в разговоре

**Методы в коде:**
- `getCompanies()` - Получить список компаний
- `getCompany(companyId)` - Получить компанию по ID
- `createCompany(company)` - Создать компанию
- `updateCompany(companyId, company)` - Обновить компанию

**Использование:**

- Управление разговорами
- Закрытие завершенных разговоров
- История коммуникаций

---

### 17. **События (Events)**

**Endpoints:**

- `GET /api/v4/events` - Список событий
- `GET /api/v4/events/{id}` - Получить событие по ID
- `GET /api/v4/events/types` - Получить типы событий

**Данные:**

- `id` - ID события
- `type` - Тип события
- `entity_id` - ID сущности
- `entity_type` - Тип сущности
- `created_at` - Дата создания
- `user_id` - ID пользователя

**Методы в коде:**
- `getCompanies()` - Получить список компаний
- `getCompany(companyId)` - Получить компанию по ID
- `createCompany(company)` - Создать компанию
- `updateCompany(companyId, company)` - Обновить компанию

**Использование:**

- Отслеживание событий в CRM
- Аналитика действий
- Аудит системы

---

### 18. **Списки (Lists)**

**Endpoints:**

- `GET /api/v4/lists` - Список списков
- `GET /api/v4/lists/{id}` - Получить список
- `POST /api/v4/lists` - Добавить список
- `PATCH /api/v4/lists` - Обновить списки (массово)
- `PATCH /api/v4/lists/{id}` - Обновить список
- `GET /api/v4/lists/{id}/elements` - Получить элементы списка
- `GET /api/v4/lists/{id}/elements/{element_id}` - Получить элемент списка
- `POST /api/v4/lists/{id}/elements` - Добавить элементы в список
- `PATCH /api/v4/lists/{id}/elements` - Обновить элементы (массово)
- `PATCH /api/v4/lists/{id}/elements/{element_id}` - Обновить элемент

**Данные:**

- `id` - ID списка
- `name` - Название списка
- `type` - Тип списка
- `elements` - Элементы списка

**Методы в коде:**
- `getCompanies()` - Получить список компаний
- `getCompany(companyId)` - Получить компанию по ID
- `createCompany(company)` - Создать компанию
- `updateCompany(companyId, company)` - Обновить компанию

**Использование:**

- Работа со списками контактов
- Сегментация клиентов
- Массовые операции

---

### 19. **Связи между сущностями (Links)**

**Endpoints:**

- `GET /api/v4/{entity_type}/{entity_id}/links` - Список связей сущности
- `POST /api/v4/{entity_type}/{entity_id}/links` - Создать связь
- `DELETE /api/v4/{entity_type}/{entity_id}/links` - Удалить связь

**Данные:**

- `to_entity_id` - ID связанной сущности
- `to_entity_type` - Тип связанной сущности

**Методы в коде:**
- `getCompanies()` - Получить список компаний
- `getCompany(companyId)` - Получить компанию по ID
- `createCompany(company)` - Создать компанию
- `updateCompany(companyId, company)` - Обновить компанию

**Использование:**

- Связывание сделок с контактами
- Связывание компаний с контактами
- Построение связей между сущностями

---

### 20. **Роли пользователей (Roles)**

**Endpoints:**

- `GET /api/v4/users/roles` - Список ролей
- `GET /api/v4/users/roles/{id}` - Получить роль по ID
- `POST /api/v4/users/roles` - Добавить роль
- `PATCH /api/v4/users/roles/{id}` - Обновить роль
- `DELETE /api/v4/users/roles/{id}` - Удалить роль

**Данные:**

- `id` - ID роли
- `name` - Название роли
- `rights` - Права доступа

**Методы в коде:**
- `getCompanies()` - Получить список компаний
- `getCompany(companyId)` - Получить компанию по ID
- `createCompany(company)` - Создать компанию
- `updateCompany(companyId, company)` - Обновить компанию

**Использование:**

- Управление правами доступа
- Назначение ролей пользователям

---

### 21. **Виджеты (Widgets)**

**Endpoints:**

- `GET /api/v4/widgets` - Список виджетов
- `GET /api/v4/widgets/{code}` - Получить виджет по коду
- `POST /api/v4/widgets/{code}/install` - Установить виджет в аккаунт
- `POST /api/v4/widgets/{code}/uninstall` - Удалить виджет из аккаунта

**Данные:**

- `code` - Код виджета
- `name` - Название виджета
- `version` - Версия виджета
- `settings` - Настройки виджета

**Методы в коде:**
- `getCompanies()` - Получить список компаний
- `getCompany(companyId)` - Получить компанию по ID
- `createCompany(company)` - Создать компанию
- `updateCompany(companyId, company)` - Обновить компанию

**Использование:**

- Интеграция виджетов
- Кастомизация интерфейса

---

### 22. **Salesbot**

**Endpoints:**

- `POST /api/v4/salesbot/launch` - Запустить Salesbot

**Данные:**

- `bot_id` - ID бота
- `entity_id` - ID сущности
- `entity_type` - Тип сущности

**Методы в коде:**
- `getCompanies()` - Получить список компаний
- `getCompany(companyId)` - Получить компанию по ID
- `createCompany(company)` - Создать компанию
- `updateCompany(companyId, company)` - Обновить компанию

**Использование:**

- Автоматизация продаж
- Чат-боты

---

## 🔄 Webhooks (Входящие события)

**Endpoint:** `POST /api/crm/webhook` (наш endpoint)

**Типы событий:**

- `leads` - Изменения в сделках
- `contacts` - Изменения в контактах
- `customers` - Изменения в клиентах
- `tasks` - Изменения в задачах
- `messages` - Сообщения
- `calls` - Звонки
- `companies` - Изменения в компаниях

**Формат:**

```json
{
  "account": {
    "base_domain": "subdomain.kommo.com"
  },
  "leads": {
    "status": [{ "id": 123, "status_id": 456, ... }],
    "add": [{ "id": 123, ... }],
    "update": [{ "id": 123, ... }],
    "delete": [{ "id": 123 }]
  }
}
```

**Методы в коде:**

- `KommoAPI.parseWebhook(payload)` - Парсинг webhook

**Использование:**

- Автоматическая обработка событий
- Триггеры для агентов
- Синхронизация данных

---

## 📊 Каналы связи (Channels)

**Примечание:** Каналы не имеют отдельного API endpoint, но определяются через:

- Типы примечаний (`note_type`):
  - `mail_message` - Email
  - `sms_in` / `sms_out` - SMS
  - `whatsapp_message` - WhatsApp
  - `call_in` / `call_out` - Звонки
- Источники сделок (`source` в custom fields)
- Webhooks для сообщений

**Использование:**

- Настройка каналов для агента
- Фильтрация по каналам
- Статистика по каналам

---

## 🎯 Действия (Actions)

**Доступные действия через API:**

1. **Создание/обновление сделок:**
   - `createLead()` - Создать сделку
   - `updateLead()` - Обновить сделку
   - Перемещение по этапам (через `updateLead` с `status_id`)

2. **Создание/обновление контактов:**
   - `createContact()` - Создать контакт
   - `updateContact()` - Обновить контакт

3. **Работа с задачами:**
   - `createTask()` - Создать задачу
   - `updateTask()` - Обновить задачу

4. **Отправка сообщений:**
   - `sendEmailFromLead()` - Отправить email
   - `createNote()` - Создать примечание (любого типа)

5. **Работа с звонками:**
   - `createCallNote()` - Создать примечание о звонке

6. **Работа со встречами:**
   - `createMeetingNote()` - Создать примечание о встрече

**Использование:**

- Настройка триггеров
- Автоматизация действий
- Цепочки действий

---

## 📝 Резюме для синхронизации

### Что нужно синхронизировать (только по кнопке):

1. **Воронки (Pipelines)** - `getPipelines()`
2. **Этапы (Stages)** - вложены в `getPipelines()`
3. **Кастомные поля сделок** - `getCustomFields()` с `element_type: 'lead'`
4. **Кастомные поля контактов** - `getCustomFields()` с `element_type: 'contact'`
5. **Пользователи** - `getUsers()`
6. **Каналы** - определяются через типы примечаний и источники

### Что работает автоматически (через webhooks):

- События по сделкам
- События по контактам
- Сообщения
- Звонки
- Задачи

---

## 🔧 Рекомендации по реализации

### 1. Структура данных для кэша

```typescript
interface KommoSyncCache {
  pipelines: Array<{
    id: number;
    name: string;
    stages: Array<{
      id: number;
      name: string;
      sort: number;
      color: string;
      type: number;
    }>;
  }>;
  dealFields: Array<{
    id: number;
    name: string;
    type: string;
    enums?: Record<string, string>;
  }>;
  contactFields: Array<{
    id: number;
    name: string;
    type: string;
    enums?: Record<string, string>;
  }>;
  users: Array<{
    id: number;
    name: string;
    email: string;
  }>;
  channels: Array<{
    id: string;
    name: string;
    type: string;
  }>;
}
```

### 2. API endpoints для синхронизации

```
POST /api/kommo/sync                    # Запустить синхронизацию
GET  /api/kommo/sync/status             # Статус синхронизации
GET  /api/kommo/pipelines               # Получить воронки (из кэша)
GET  /api/kommo/stages                   # Получить этапы (из кэша)
GET  /api/kommo/deal-fields              # Получить поля сделок (из кэша)
GET  /api/kommo/contact-fields           # Получить поля контактов (из кэша)
GET  /api/kommo/users                    # Получить пользователей (из кэша)
GET  /api/kommo/channels                 # Получить каналы (из кэша)
```

### 3. Действия для триггеров/цепочек

```typescript
type KommoAction =
  | { type: "create_task"; task: KommoTask }
  | { type: "update_lead"; leadId: number; data: Partial<KommoLead> }
  | { type: "update_contact"; contactId: number; data: Partial<KommoContact> }
  | { type: "move_lead"; leadId: number; statusId: number }
  | { type: "send_email"; leadId: number; email: KommoEmailMessage }
  | { type: "create_note"; note: KommoNote };
```

---

## 🎯 Что можно делать через наш интерфейс

### ✅ Реализовано в проекте

#### 1. **Работа с письмами (Email)**

**Отправка писем:**

- ✅ Отправка писем клиентам через AI-агента
- ✅ Создание примечания типа `mail_message` или `outgoing_email`
- ✅ Kommo реально отправляет письмо клиенту через свою email-интеграцию
- ✅ Письмо доставляется на email адрес клиента
- ✅ История сохраняется в сделке

**Чтение писем:**

- ✅ Получение всех примечаний по сделке (включая письма)
- ✅ Фильтрация по типу `mail_message`, `outgoing_email`, `incoming_email`
- ✅ Чтение входящих писем от клиентов
- ✅ Просмотр истории переписки

**Реализация:**

```typescript
// Отправка письма
await kommoApi.sendEmailFromLead(leadId, {
  to: ["client@example.com"],
  subject: "Ответ на ваш вопрос",
  html: "<p>Текст письма</p>",
  text: "Текст письма",
  from: "agent@company.com",
});

// Чтение писем
const notes = await kommoApi.getNotesByEntity(leadId, "leads");
const emails = notes.filter(
  (note) =>
    note.note_type === "mail_message" ||
    note.note_type === "outgoing_email" ||
    note.note_type === "incoming_email",
);
```

#### 2. **Работа со сделками**

- ✅ Получение данных сделки (`getLead()`)
- ✅ Создание сделки (`createLead()`)
- ✅ Обновление сделки (`updateLead()`)
- ✅ Поиск сделок (`searchLeads()`)
- ✅ Получение списка сделок (`getLeads()`)
- ✅ Обновление полей сделки
- ✅ Перемещение по этапам (через `updateLead` с `status_id`)

#### 3. **Работа с контактами**

- ✅ Получение контакта (`getContact()`)
- ✅ Создание контакта (`createContact()`)
- ✅ Обновление контакта (`updateContact()`)
- ✅ Поиск контактов (`searchContacts()`)

#### 4. **Работа с воронками и этапами**

- ✅ Получение воронок (`getPipelines()`)
- ✅ Получение этапов (вложено в `getPipelines()`)
- ✅ Синхронизация воронок и этапов

#### 5. **Работа с задачами**

- ✅ Создание задачи (`createTask()`)
- ✅ Обновление задачи (`updateTask()`)
- ✅ Получение задач по сущности (`getTasksByEntity()`)

#### 6. **Работа с примечаниями**

- ✅ Создание примечания (`createNote()`)
- ✅ Получение примечаний по сущности (`getNotesByEntity()`)
- ✅ Создание примечания о звонке (`createCallNote()`)
- ✅ Создание примечания о встрече (`createMeetingNote()`)

#### 7. **Работа с кастомными полями**

- ✅ Получение всех полей (`getCustomFields()`)
- ✅ Создание поля (`createCustomField()`)
- ✅ Фильтрация по типу сущности (`element_type`)

#### 8. **Работа с пользователями**

- ✅ Получение списка пользователей (`getUsers()`)

#### 9. **Webhooks**

- ✅ Обработка входящих webhooks (`KommoAPI.parseWebhook()`)
- ✅ Автоматическая обработка событий по сделкам, контактам, задачам, сообщениям

---

### ⚠️ Не реализовано, но доступно через API

#### 1. **Компании (Companies)**

- Получение списка компаний
- Создание/обновление компаний
- Связывание с контактами и сделками

#### 2. **Теги (Tags)**

- Получение тегов
- Добавление/обновление тегов для сущностей

#### 3. **Аккаунт (Account)**

- Получение информации об аккаунте
- Проверка подключения

#### 4. **Входящие лиды (Incoming Leads)**

- Обработка входящих заявок
- Принятие/отклонение лидов
- Привязка к сделкам

#### 5. **Источники (Sources)**

- Управление источниками сделок
- Аналитика по источникам

#### 6. **Шаблоны (Templates)**

- Управление шаблонами писем
- WhatsApp шаблоны

#### 7. **Разговоры (Conversations)**

- Управление разговорами
- Закрытие разговоров

#### 8. **События (Events)**

- Отслеживание событий
- Аналитика действий

#### 9. **Списки (Lists)**

- Работа со списками контактов
- Сегментация

#### 10. **Связи между сущностями (Links)**

- Связывание сущностей
- Построение связей

#### 11. **Роли пользователей (Roles)**

- Управление ролями
- Права доступа

#### 12. **Виджеты (Widgets)**

- Установка/удаление виджетов

#### 13. **Salesbot**

- Запуск Salesbot

---

## 📧 Детальная работа с электронными письмами

### Отправка письма через наш интерфейс

**Через AI-агента:**

1. Агент получает входящее письмо от клиента
2. Агент анализирует письмо и генерирует ответ
3. Агент отправляет ответ через `sendEmailFromLead()`
4. Kommo реально отправляет письмо клиенту
5. Письмо сохраняется в истории сделки как примечание

**Через триггеры/цепочки:**

1. Сделка переходит на определенный этап
2. Срабатывает триггер
3. Выполняется действие "Отправить письмо"
4. Письмо отправляется через `sendEmailFromLead()`
5. Письмо сохраняется в истории

**Через наш API:**

```typescript
POST /api/integrations/kommo/messages/send
{
  "orgId": "uuid",
  "dealId": "123",
  "channel": "email",
  "message": {
    "subject": "Ответ на ваш вопрос",
    "body": "Текст письма",
    "attachments": [
      { "url": "https://...", "name": "file.pdf" }
    ]
  }
}
```

### Чтение писем через наш интерфейс

**Получение всех писем по сделке:**

```typescript
// Через наш API
GET /api/crm/kommo?action=lead&id=123
// Возвращает сделку со всеми данными

// Затем получаем примечания
const notes = await kommoApi.getNotesByEntity(leadId, 'leads')
const emails = notes.filter(note =>
  note.note_type === 'mail_message' ||
  note.note_type === 'outgoing_email' ||
  note.note_type === 'incoming_email'
)
```

**Через webhooks:**

- Kommo отправляет webhook при получении нового письма
- Наш сервис обрабатывает webhook
- Письмо автоматически добавляется в историю сделки
- Агент может автоматически ответить на письмо

**Структура письма в примечании:**

```typescript
{
  id: 12345,
  entity_id: 67890, // ID сделки
  entity_type: 'leads',
  note_type: 'mail_message', // или 'outgoing_email', 'incoming_email'
  params: {
    text: 'Текст письма в plain text',
    html: '<p>Текст письма в HTML</p>',
    subject: 'Тема письма',
    from: 'client@example.com',
    to: 'agent@company.com',
    status: 1, // 1 = отправлено, 0 = черновик
    attachments: [] // Вложения
  },
  created_at: 1234567890,
  created_by: 123
}
```

---

## 🔧 Полный список методов API (официальная документация)

Согласно официальной документации Kommo API (https://developers.kommo.com/docs/about-kommo-api), доступны следующие методы:

### Account

- ✅ Get the account information

### Leads

- ✅ Get a leads list
- ✅ Get a single lead
- ✅ Add leads
- ✅ Update a lead
- ✅ Update a bunch of leads
- ✅ Complex addition leads (with contacts and companies)

### Pipelines and Stages

- ✅ Get a pipelines list
- ✅ Get a single pipeline
- ✅ Add pipelines
- ✅ Edit a pipeline
- ✅ Delete a pipeline
- ✅ Get a pipeline stages list
- ✅ Get a single pipeline stage
- ✅ Add stages
- ✅ Edit stages
- ✅ Delete a stage
- ✅ Stage colors codes

### Contacts

- ✅ Get a contacts list
- ✅ Get a contact
- ✅ Add contacts
- ✅ Update a contact
- ✅ Update a bunch of contacts

### Users and roles

- ✅ Get a users list
- ✅ Get a user
- ✅ Add users
- ✅ Get a user roles list
- ✅ Get a role
- ✅ Add roles
- ✅ Edit a role
- ✅ Delete a role

### Tags

- ⚠️ Get a list of entity tags
- ⚠️ Add tags for a particular entity type
- ⚠️ Update tags of a single entity
- ⚠️ Update tags of multiple entities

### Custom fields

- ✅ Get a custom fields list
- ✅ Get an entity custom field by ID
- ✅ Add custom fields

### Incoming leads

- ⚠️ Incoming leads list
- ⚠️ Getting an incoming lead by its UID
- ⚠️ Adding incoming leads from the type sip (call)
- ⚠️ Adding incoming leads from the type form
- ⚠️ Accepting incoming leads
- ⚠️ Declining incoming leads
- ⚠️ Linking incoming leads
- ⚠️ Incoming leads summary information
- ⚠️ Metadata objects description

### Tasks

- ✅ Tasks List
- ✅ Getting a task by its ID
- ✅ Adding tasks
- ✅ Editing tasks
- ✅ Editing a single task

### Sources

- ⚠️ Get a list of sources
- ⚠️ Get a source by its ID
- ⚠️ Add sources
- ⚠️ Update a source
- ⚠️ Update sources
- ⚠️ Delete a source
- ⚠️ Delete sources
- ⚠️ Get Website chat button objects
- ⚠️ Get a Website chat button
- ⚠️ Add Website chat button
- ⚠️ Connect online chat to Website chat button
- ⚠️ Update Website chat button

### Salesbot

- ⚠️ Launch Salesbot

### Companies

- ⚠️ Companies list
- ⚠️ Getting a company by its ID
- ⚠️ Add companies
- ⚠️ Updating companies
- ⚠️ Updating a company

### Templates

- ⚠️ Get a list of templates
- ⚠️ Get a template by ID
- ⚠️ Add templates
- ⚠️ Edit templates
- ⚠️ Submit a WhatsApp template for moderation
- ⚠️ Edit a WhatsApp template status
- ⚠️ Delete templates
- ⚠️ Delete a template

### Webhooks

- ✅ List of webhooks
- ✅ Adding a webhook
- ✅ Deleting a webhook
- ✅ Webhook events

### Conversations

- ⚠️ Get a conversation by ID
- ⚠️ Close a conversation by ID

### Widgets

- ⚠️ Widgets List
- ⚠️ Getting the widget info by its code
- ⚠️ Installing the widget into the account
- ⚠️ Uninstalling widget

### Lists

- ⚠️ Get a list of lists
- ⚠️ Get a list
- ⚠️ Adding lists
- ⚠️ Editing lists
- ⚠️ Editing a list
- ⚠️ Get list elements
- ⚠️ Get a list element
- ⚠️ Adding list elements
- ⚠️ Editing list elements
- ⚠️ Editing a list element

### Events

- ⚠️ Events list
- ⚠️ Getting an event by its ID
- ⚠️ Getting events types
- ⚠️ Events types

### Notes ⭐ КРИТИЧНО

- ✅ Getting a particular entity's notes list by the entity ID
- ✅ Notes list by the entity type
- ✅ Get a note by its ID
- ✅ Adding notes
- ✅ Editing notes
- ✅ Editing a note
- ✅ Notes types

### Calls

- ✅ Add calls

### OAuth 2.0

- ✅ Get/renew an access token

### Links between entities

- ⚠️ Linked entities list
- ⚠️ Linking entities
- ⚠️ Unlinking entities

**Легенда:**

- ✅ Реализовано в проекте
- ⚠️ Не реализовано, но доступно через API

---

**Последнее обновление:** 2025-01-26  
**Источник документации:** https://developers.kommo.com/docs/about-kommo-api
