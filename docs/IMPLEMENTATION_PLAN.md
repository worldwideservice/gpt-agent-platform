...
## 7. Product Analytics & GTM Enablement

### 7.1 Инструментарий аналитики
- [x] Вынести `ProductAnalyticsProvider` (Segment + PostHog) и подключить его в `app/layout.tsx` и `app/manage/[tenantId]/layout.tsx`.
- [x] Описать ключевые продуктовые метрики и события (см. `docs/PRODUCT_ANALYTICS.md`).
- [ ] Добавить автоматическую идентификацию пользователей (identify/alias) после подключения auth API.
- [ ] Подготовить дашборды в PostHog/Segment Personas по воронке активации.

### 7.2 Контент для клиентов
- [x] Обновить публичный FAQ (`app/faq/page.tsx`) и навигацию `PublicHeader`.
- [x] Создать гайд для поддержки и Customer Success (`docs/support.md`).
- [ ] Сверстать раздел «Обновления продукта» в публичном сайте (история релизов).
- [ ] Добавить внутрикабинетный центр обучения с онбординг-чеклистом.

### 7.3 GTM процессы
- [x] Описать тарифные пакеты, онбординг и email-цепочки (см. `docs/GTM_PLAYBOOK.md`).
- [x] Зафиксировать процессы релиз-нотов и SLA (см. `docs/GTM_PLAYBOOK.md`).
- [ ] Согласовать с маркетингом календарь релизов и маркетинговых кампаний.
- [ ] Настроить автоматическую отправку release notes в PostHog/Segment (webhook → email).
